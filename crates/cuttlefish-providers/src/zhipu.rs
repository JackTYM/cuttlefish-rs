...
//!
//! Provides access to Zhipu's GLM models via their OpenAI-compatible API.
...
const API_BASE: &str = "https://open.bigmodel.cn/api/paas/v4/chat/completions";

/// Zhipu (GLM) model provider.
///
/// Sends requests to Zhipu's chat completions API using OpenAI-compatible format.
pub struct ZhipuProvider {
    /// HTTP client.
    client: Client,
    /// API key.
    api_key: String,
    /// Model ID (e.g., `glm-4-flash`, `glm-4`).
    model: String,
}

impl ZhipuProvider {
    /// Create a new provider, reading API key from `ZHIPU_API_KEY` environment variable.
    ///
    /// # Errors
    /// Returns an error if `ZHIPU_API_KEY` is not set.
    pub fn new(model: impl Into<String>) -> Result<Self, ProviderError> {
        let api_key = std::env::var("ZHIPU_API_KEY")
            .map_err(|_| ProviderError("ZHIPU_API_KEY environment variable not set".to_string()))?;
    ...
    /// Parse the response from Zhipu's API (OpenAI-compatible format).
...
impl ModelProvider for ZhipuProvider {
    ...
    async fn complete(
        ...
        debug!("Sending request to Zhipu API, model={}", self.model);
...
mod tests {
    ...
    fn test_provider_name() {
        let provider = ZhipuProvider::with_api_key("test-key", "glm-4-flash");
    ...
    fn test_parse_response() {
        let provider = ZhipuProvider::with_api_key("test-key", "glm-4");
        let resp_json = json!({
            "choices": [{
                "message": {
                    "role": "assistant",
