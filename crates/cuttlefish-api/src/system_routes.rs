    ...
    pub fn with_provider_registry(mut self, registry: Arc<ProviderRegistry>) -> Self {
        ...
        let providers: Vec<ProviderConfig> = registry
            ...
            .map(|name| {
                ...
                // Detect provider type from name (handles names like "bedrock-fast")
                let base_type = if name.starts_with("bedrock") {
                    "bedrock"
                ...
                ProviderConfig {
                    id: name.to_string(),
                    name: name.to_string(),
                    provider_type: base_type.to_string(),
                    enabled: true,
