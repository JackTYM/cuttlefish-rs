...
<template>
  <div class="p-6 max-w-4xl">
    ...
    <!-- API Keys Section -->
    <section class="mb-8 p-6 bg-gray-900 border border-gray-800 rounded-xl">
      ...
      <div class="flex items-center gap-3">
        <input 
          ...
          readonly
          class="flex-1 px-4 py-2 bg-gray-800 border border-gray-700 rounded-lg text-gray-400 font-mono text-sm focus:outline-none"
        />
        <button 
          @click="copyApiKey" 
        ...
        <button 
          ...
          class="px-4 py-2 bg-red-900/30 hover:bg-red-900/50 text-red-400 border border-red-900/50 rounded-lg transition-colors"
        >
          Regenerate
        </button>
      </div>
    ...
    <section class="mb-8 p-6 bg-gray-900 border border-gray-800 rounded-xl">
      ...
      <div class="space-y-4">
        <div v-for="(model, category) in settings.modelCategory" :key="category" class="flex items-center gap-4">
          <label class="w-32 text-gray-400 capitalize text-sm font-medium">{{ category }}</label>
          <select 
            v-model="settings.modelCategory[category]"
            class="flex-1 px-4 py-2 bg-gray-800 border border-gray-700 rounded-lg text-white text-sm focus:outline-none focus:border-cyan-500 transition-colors"
          >
      ...
      <div class="grid md:grid-cols-3 gap-4">
        <div>
          ...
          <input 
            ...
            class="w-full px-4 py-2 bg-gray-800 border rounded-lg text-white text-sm focus:outline-none transition-colors"
    ...
    <div class="flex justify-end">
      <button 
        ...
        class="px-6 py-3 bg-cyan-600 hover:bg-cyan-500 disabled:opacity-50 disabled:cursor-not-allowed text-white rounded-lg font-medium transition-colors flex items-center gap-2"
      >
